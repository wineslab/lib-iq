...
def read_binary_data(file_path: str, dtype: np.dtype, max_rows: int = None) -> np.ndarray:
    """
...
def process_binary_file(file_path: str,
    ...
    for file_name, rows in saved_files_info:
        if rows != updated_n_samples:
            print(f"Warning: The file '{file_name}' contains {rows} samples instead of the expected {updated_n_samples}.")
    ...
    with open(output_file, 'w', newline='') as outfile:
        ...
        for file_path in csv_files:
            ...
            if base_name in seen_base_names:
                print(f"Warning: Duplicate base file name detected: '{base_name}'. Skipping file '{file_path}'.")
                continue
...
def create_dataset_from_bin(files: Dict[str, int],
    ...
    for file_path, ground_truth in files.items():
        result = pool.apply_async(
            process_binary_file,
            args=(file_path, ground_truth, output_path, input_vector, num_files, dtype,
                  extraction_window, moving_avg_window)
        )
    ...
    if not all(x == updated_samples_list[0] for x in updated_samples_list):
        print("Warning: Not all binary files produced the same number of samples after cropping.")
    
    combine_csv_files_with_check(csv_files, combined_output_path, updated_samples_list[0])
