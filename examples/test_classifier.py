...
def main():
    ...
    classification_model = Classifier(time_window=1, input_vector=1536, moving_avg_window=30, extraction_window=600)

    #start training

    create_dataset_from_bin(files, 1, OUTPUT_PATH, f"{COMBINED_CSV_FILE_PATH}combined_output.csv", classification_model.time_window, classification_model.input_vector, classification_model.moving_avg_window)                
    x_train, x_test, y_train, y_test = preprocess_data(f"{COMBINED_CSV_FILE_PATH}combined_output.csv", 0.0001)
    print(x_train.shape)
    print(x_test.shape)

    wf.plot_waterfall("/home/user/Desktop/libiq/examples/test_results/iq_samples/csv/combined_output.csv", plots_mode="", fft_size=classification_model.extraction_window, path=f'{PLOTS_PATH}waterfall_train.pdf')
    ...
    create_dataset_from_bin(files_test, 1, OUTPUT_PATH, f"{COMBINED_CSV_FILE_PATH}combined_output.csv", classification_model.time_window, classification_model.input_vector, classification_model.moving_avg_window)                
    x_train, x_test, y_train, y_test = preprocess_data(f"{COMBINED_CSV_FILE_PATH}combined_output.csv", 0.9999)
    print(x_train.shape)
    print(x_test.shape)
    wf.plot_waterfall("/home/user/Desktop/libiq/examples/test_results/iq_samples/csv/combined_output.csv", plots_mode="", fft_size=classification_model.extraction_window, path=f'{PLOTS_PATH}waterfall_test.pdf')
